...
class SQ:
    ...
    def __init__(self, image_manager, frame, index, id = None, cords = None):
        ...
        self.point_one.forcePos = self.forcePos(self.point_one, self.point_three, self.point_two)
        self.point_two.forcePos = self.forcePos(self.point_two, self.point_four, self.point_one)
        self.point_three.forcePos = self.forcePos(self.point_three, self.point_one, self.point_four)
        self.point_four.forcePos = self.forcePos(self.point_four, self.point_two, self.point_three)
    ...
    def get_starting_locs(self, cords):
        if not cords:
            ...
            deep_x, deep_y = cords.w + cords.x - self.ADJUST, cords.h + cords.y - self.ADJUST
    ...
    def forcePos(self, target_node: MovingObject, connected_x: MovingObject, connected_y: MovingObject, outside_event: bool = False):        
        def forcePos(updated_cursor_x: int, updated_cursor_y: int, outside_event: bool = False):
    ...
    def cv_cords(self):
        locinfo = self.getLocInfo()
        return locinfo.get_adjusted_dims(
            self.x_scale,
            self.y_scale
        )
